...
def normalize_contest_name(raw_name: str) -> str:
    ...
    Transformations applied in order:
      ...
      2. Strip geographic qualifiers: ", State of Illinois"
      3. Strip party suffixes: " - D*", " - R*", " - G*", " - GP", etc.
      4. Strip trailing asterisks
      5. Strip "Unexpired N Year" phrases anywhere in the name
      6. Strip term-length suffixes: "Full 4 Year Term", "4 Year Term", etc.
      7. Uppercase
      8. Gender-neutral titles: committeeman/woman → committeeperson, etc.
      9. Expand township abbreviations in committeeperson names
      10. Spell out ordinal numerals: "81st" → "EIGHTY-FIRST"
    ...
    # 2. Strip geographic qualifiers before party suffix stripping
    name = re.sub(r",?\s*State of Illinois\s*", "", name, flags=re.IGNORECASE).strip()

    # 3. Strip party suffixes (D, R, G, GP) before uppercasing to catch mixed case
    name = re.sub(r"\s*-\s*(GP|[DRG])\*?\s*$", "", name, flags=re.IGNORECASE)

    # 4. Strip any remaining trailing asterisk
    name = re.sub(r"\s*\*\s*$", "", name).strip()

    # 5. Strip "Unexpired N Year" anywhere in the name
    ...
    # 6. Strip term-length suffixes (with optional leading comma or dash)
    ...
    # 9. Expand DuPage County township abbreviations in precinct committeeperson
